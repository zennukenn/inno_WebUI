...
if [ "$WITH_GPU" = "true" ] || [ "$WITH_GPU" = "gpu" ]; then
    ...
    docker-compose -f docker-compose.single.yml --profile gpu up -d
else
    echo "🚀 Starting basic mode..."
    docker-compose -f docker-compose.single.yml up -d inno-webui
fi
...
echo "  Restart: ./scripts/docker_single.sh $WITH_GPU rebuild"
...
if curl -f http://localhost/health &>/dev/null; then
    ...
    docker-compose -f docker-compose.single.yml logs --tail=20 inno-webui
fi
